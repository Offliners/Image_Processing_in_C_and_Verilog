...
int main(int argc, char *argv[])
{
    ...
    BMPImage *out = bmp_planar_roundtrip(img);
    free_bmp_image(img);
    if(!out)
    {
        printf(RED_COLOR "Planar round-trip failed.\n" ENDL_COLOR);
        return FUNC_FAIL;
    }

    printf(YELLOW_COLOR "Output Image (planar B/G/R split + merge, identity)\n" ENDL_COLOR);
    show_bmp_info(out);

    BYTE u08Ret = write_image("../output.bmp", out);
    free_bmp_image(out);
    return u08Ret;
}
...
BMPImage *read_image(const char *filename)
{
    ...
    if(!fp)
        return NULL;
    BMPImage *im = read_bmp(fp, &error_record);
    fclose(fp);
    if(error_record || !im)
        return NULL;
    return im;
