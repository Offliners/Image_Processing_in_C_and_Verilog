...
// 3-line BGR buffer; dilation on B-channel gray vs BIN_THRESHOLD (legacy).
...
// Last body byte of 3×3 window (clipped to image); do NOT use obi on borders — that fires before lower rows are buffered.
wire [31:0] win_max_py = (oy_b + 32'd1 < `BMP_HEIGHT) ? (oy_b + 32'd1) : (`BMP_HEIGHT - 32'd1);
wire [31:0] win_max_px = (ox_b + 32'd1 < `BMP_WIDTH) ? (ox_b + 32'd1) : (`BMP_WIDTH - 32'd1);
wire [31:0] need_k = (win_max_py * `BMP_WIDTH + win_max_px) * 32'd3 + 32'd2;
wire out_ok = (st == S_STREAM) && (obi <= PIXEL_LAST) && (ibi > need_k);

reg [`BYTE_WIDTH-1:0] out_val;
integer ny_i, nx_i;
integer cy_i, cx_i;
reg [`BYTE_WIDTH-1:0] res_c;
...
always @(*) begin
    ...
    if (st == S_STREAM && obi <= PIXEL_LAST && out_ok) begin
        if (bod) begin
            res_c = 8'd0;
            for (ny_i = -1; ny_i <= 1; ny_i = ny_i + 1) begin
                for (nx_i = -1; nx_i <= 1; nx_i = nx_i + 1) begin
                    cy_i = oy_b + ny_i;
                    cx_i = ox_b + nx_i;
                    if (cy_i >= 0 && cy_i < `BMP_HEIGHT && cx_i >= 0 && cx_i < `BMP_WIDTH) begin
                        if (f_pix(cy_i, cx_i, 32'd0) > `BIN_THRESHOLD)
                            res_c = 8'd255;
                    end
                end
            end
