...
int main(int argc, char *argv[])
{
    ...
    BMPImage *out = bmp_downscale_half_box(img);
    free_bmp_image(img);
    if(!out)
    {
        printf(RED_COLOR "Downscale failed (need even width/height >= 2).\n" ENDL_COLOR);
        return FUNC_FAIL;
    }

    printf(YELLOW_COLOR "Output Image (half size, 2x2 box average)\n" ENDL_COLOR);
    show_bmp_info(out);

    u08Ret |= write_image("../output.bmp", out);
    free_bmp_image(out);
