...
`include "LOAD_BMP.v"
`ifdef GATE
`include "DOWN_SCALE_SYN.v"
`else
`include "DOWN_SCALE.v"
`endif
`include "BMP_ROM.v"
...
always @(posedge clk) begin
    if (!rst_n)
        ...
        if (sim_cycle_cnt % 1000 == 0)
            $display("[IMAGE DOWNSCALE] %0d cycles", sim_cycle_cnt);
    end
end

initial begin
`ifdef GATE
    $sdf_annotate("DOWN_SCALE_SYN.sdf", DOWN_SCALE1);
`endif
`ifdef __ICARUS__
`ifndef GATE
    $dumpfile("DOWN_SCALE.vcd");
    $dumpvars;
`endif
`else
    $fsdbDumpfile("DOWN_SCALE.fsdb");
    $fsdbDumpvars;
`endif
end
...
BMP_LWORD_RAM #(.NUM_WORDS(`BMP_RAM_IN_NUM_WORDS)) BMP_RAM_IN(
    .clk(clk),
...
always @(posedge done)begin
    ...
    for(bi = 0; bi < `BMP_OUT_TOTAL_SIZE; bi = bi + 1) begin
        wi = bi >> 2;
        case (bi[1:0])
