...
`include "BGR2GRAY.v"
`ifdef GATE
`include "CONNECTED_COMPONENTS_SYN.v"
`else
`include "CONNECTED_COMPONENTS.v"
`endif
`include "BMP_ROM.v"
...
always @(posedge clk) begin
    if (!rst_n)
        ...
        if (sim_cycle_cnt % 1000 == 0)
            $display("[CONNECTED COMPONENTS] %0d cycles", sim_cycle_cnt);
    end
end
...
CONNECTED_COMPONENTS CONNECTED_COMPONENTS1(
    .clk(clk),
    .rst_n(rst_n),
    .start(dil_start),
    .RAM_in_out(ram_in_out),
