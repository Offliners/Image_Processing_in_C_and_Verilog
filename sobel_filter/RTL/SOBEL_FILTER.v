                ...
                for(yi = 0; yi < `BMP_HEIGHT; yi = yi + 1) begin
                    for(xi = 0; xi < `BMP_WIDTH; xi = xi + 1) begin
                        ...
                        if(yi == 0 || xi == 0 || yi == `BMP_HEIGHT - 1 || xi == `BMP_WIDTH - 1) begin
                            ...
                            gx = 0;
                            gy = 0;
                            gx = gx - gray_data[(yi - 1) * `BMP_WIDTH + (xi - 1)];
                            gx = gx + gray_data[(yi - 1) * `BMP_WIDTH + (xi + 1)];
                            gx = gx - (gray_data[yi * `BMP_WIDTH + (xi - 1)] << 1);
                            gx = gx + (gray_data[yi * `BMP_WIDTH + (xi + 1)] << 1);
                            gx = gx - gray_data[(yi + 1) * `BMP_WIDTH + (xi - 1)];
                            gx = gx + gray_data[(yi + 1) * `BMP_WIDTH + (xi + 1)];

                            gy = gy + gray_data[(yi - 1) * `BMP_WIDTH + (xi - 1)];
                            gy = gy + (gray_data[(yi - 1) * `BMP_WIDTH + xi] << 1);
                            gy = gy + gray_data[(yi - 1) * `BMP_WIDTH + (xi + 1)];
                            gy = gy - gray_data[(yi + 1) * `BMP_WIDTH + (xi - 1)];
                            gy = gy - (gray_data[(yi + 1) * `BMP_WIDTH + xi] << 1);
                            ...
                            mag = gx + gy;
                            if(mag > 255) mag = 255;
                            out_data[base_idx] = mag[7:0];
                            out_data[base_idx + 1] = mag[7:0];
                            out_data[base_idx + 2] = mag[7:0];
                        end
                    end
                end
                write_idx <= 0;
