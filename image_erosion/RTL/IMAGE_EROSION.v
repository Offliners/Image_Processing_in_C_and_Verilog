...
// 3-line BGR buffer; erosion on B-channel gray vs BIN_THRESHOLD (legacy).
        ...
        if (bod) begin
            ...
            for (ny_i = -1; ny_i <= 1; ny_i = ny_i + 1) begin
                for (nx_i = -1; nx_i <= 1; nx_i = nx_i + 1) begin
                    cy_i = oy_b + ny_i;
                    cx_i = ox_b + nx_i;
                    if (cy_i < 0 || cy_i >= `BMP_HEIGHT || cx_i < 0 || cx_i >= `BMP_WIDTH)
                        res_c = 8'd0;
                    else if (f_pix(cy_i, cx_i, 32'd0) <= `BIN_THRESHOLD)
                        res_c = 8'd0;
                end
            end
            out_val = res_c;
