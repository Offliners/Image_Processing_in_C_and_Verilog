...
# (per module RTL/Makefile), then compare C vs RTL output with each module's compare.py.
#
...
#   RUN_RTL=0 ./run_all_content_rtl.sh        # same as --skip-rtl
#
# Gate-level simulation: use ./run_all_content_gate.sh (vcs or irun only).
#
...
run_one() {
  ...
  if [[ "$SKIP_RTL" -eq 1 ]]; then
    log "[$dir] skipping RTL simulation (--skip-rtl)"
    return 0
  fi
...
if [[ "$SKIP_RTL" -eq 1 ]]; then
  ...
  echo "Done: all modules passed (RTL, $RTL_TOOL)."
fi
exit 0
