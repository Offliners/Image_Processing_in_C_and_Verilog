...
// 3-line BGR buffer; Laplacian on B channel only (legacy behavior).
...
always @(*) begin
    ...
    if (st == S_STREAM && obi <= PIXEL_LAST && out_ok) begin
        if (bod) begin
            ...
            sum_c = 0;
            sum_c = sum_c - f_pix(oy_b - 32'd1, ox_b, 32'd0);
            sum_c = sum_c - f_pix(oy_b, ox_b - 32'd1, 32'd0);
            sum_c = sum_c + (f_pix(oy_b, ox_b, 32'd0) << 2);
            sum_c = sum_c - f_pix(oy_b, ox_b + 32'd1, 32'd0);
            sum_c = sum_c - f_pix(oy_b + 32'd1, ox_b, 32'd0);
            if (sum_c < 0)
                sum_c = -sum_c;
